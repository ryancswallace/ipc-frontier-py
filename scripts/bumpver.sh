...
### 3. Print out the git push command to run to publish the change to GitHub
###
...
GIT_REMOTE="origin"
printf "\nTo commit, tag, and push the new version run the following commands:\n"
echo "git add pyproject.toml \\"
echo "  && git commit -m \"chore: bump to version $VERSION\" \\"
echo "  && git push $GIT_REMOTE HEAD \\"
